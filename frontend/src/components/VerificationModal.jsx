...
export default function VerificationModal({ incidentId, riskScore, onClose, onResult }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!responsePhrase.trim() || !challenge.current || isSubmitting) return;
    ...
    const riskScoreBefore = riskScore;
    let riskScoreAfter = riskScore;
  ...
  const resultStyles = { PASS: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30', PARTIAL: 'bg-amber-500/20 text-amber-300 border-amber-500/30', FAIL: 'bg-rose-500/20 text-rose-300 border-rose-500/30', EXPIRED: 'bg-rose-500/20 text-rose-300 border-rose-500/30' };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="glass-panel w-full max-w-md rounded-2xl border border-slate-700 shadow-2xl p-6 space-y-4">
        ...
        {result && <div className={`p-3 rounded-xl text-xs font-semibold border ${resultStyles[result.outcome]}`}><div>{result.outcome === 'PASS' ? 'Challenge passed. Normal monitoring resumed.' : result.outcome === 'PARTIAL' && result.attempts < 2 ? 'Partial match. You may retry once.' : `Challenge ${result.outcome.toLowerCase()}.`}</div><div className="mt-1">Similarity: {Math.round(result.similarityScore * 100)}% | Risk: {result.riskScoreBefore} → {result.riskScoreAfter}</div>{result.suspicious && <div className="mt-1">Suspicious response latency: {result.latencyMs}ms (+10 risk)</div>}</div>}
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2"><button type="button" onClick={startRecording} disabled={isRecording || (!canRetry && Boolean(result))} className="flex items-center space-x-1.5 px-3 py-2 rounded-xl bg-slate-800 text-slate-300 text-xs font-medium hover:bg-slate-700 disabled:opacity-50">{isRecording ? <MicOff className="w-3.5 h-3.5" /> : <Mic className="w-3.5 h-3.5" />}<span>{isRecording ? 'Recording...' : 'Record Spoken Response'}</span></button><button type="button" onClick={() => setResponsePhrase(challengePhrase)} disabled={!canRetry && Boolean(result)} className="px-3 py-2 rounded-xl bg-slate-800 text-slate-300 text-xs font-medium hover:bg-slate-700 disabled:opacity-50">Auto-Fill Spoken Phrase</button><button type="submit" disabled={isSubmitting || (!canRetry && Boolean(result))} className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-lg transition-all disabled:opacity-50">Verify Challenge Response</button></div>
        </form>
      </div>
    </div>
  );
